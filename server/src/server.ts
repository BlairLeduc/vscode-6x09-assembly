...
const defaultSettings: IExtensionSettings = { programStart: 1024, memorySize: 32768 };
...
connection.onDidChangeConfiguration(change => {
  if (hasConfigurationCapability) {
    ...
    globalSettings = (change.settings.asm6x09 || defaultSettings) as IExtensionSettings;
  }
...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
  ...
  while ((m = pattern.exec(text)) && problems < settings.programStart) {
    problems++;
...
connection.onCompletion(
  (_: TextDocumentPositionParams): CompletionItem[] => {
