...
describe('LineParser', () => {
  ...
  it('Line with only line number returns empty token list', () => {
    ...
    expect(tokens[0]).to.deep.equal(expectedToken);
  });
  ...
  it('Line with line number, global symbol returns global symbol token', () => {
    ...
    expect(tokens[0]).to.deep.equal(expectedLineNumberToken);
    expect(tokens[1]).to.deep.equal(expectedToken);
  });
  ...
  it('Line with invalid global symbol starting with a colon followed by whitespace returns invalid global symbol token', () => {
    ...
    expect(tokens.length).to.equal(2);
    expect(tokens[0]).to.deep.equal(expectedSymbolToken);
    expect(tokens[1]).to.deep.equal(expectedColonToken);
  });
  ...
  it('* Comment at start of the line returns comment token', () => {
    ...
    expect(tokens.length).to.equal(1);
    expect(tokens[0]).to.deep.equal(expectedCommentToken);
  });
  ...
  it('Line number followed by * Comment returns comment token', () => {
    ...
    expect(tokens.length).to.equal(2);
    expect(tokens[0]).to.deep.equal(expectedLineNumberToken);
    expect(tokens[1]).to.deep.equal(expectedCommentToken);
  });
  ...
  it('Local Symbol, colon, comment returns local symbol, colon, comment tokens', () => {
    ...
    expect(tokens[0]).to.deep.equal(expectedSymbolToken);
    expect(tokens[1]).to.deep.equal(expectedColonToken);
    expect(tokens[2]).to.deep.equal(expectedCommentToken);
  });
  ...
  ['abx', 'struct'].forEach(opcode => {
    ...
    it(`Inherent opcode ${opcode} returns opcode token`, () => {
      ...
      expect(tokens.length).to.equal(1);
      expect(tokens[0]).to.deep.equal(expectedOpcodeToken);
    });
    ...
    it(`Symbol, inherent opcode ${opcode}, comment returns symbol, opcode, comment tokens`, () => {
      ...
      expect(tokens.length).to.equal(3);
      expect(tokens[0]).to.deep.equal(expectedSymbolToken);
      expect(tokens[1]).to.deep.equal(expectedOpcodeToken);
      expect(tokens[2]).to.deep.equal(expectedCommentToken);
    });
    ...
    it(`Symbol, inherent opcode ${opcode}, comment returns symbol, opcode, comment tokens`, () => {
      ...
      expect(tokens[0]).to.deep.equal(expectedSymbolToken);
      expect(tokens[1]).to.deep.equal(expectedColonToken);
      expect(tokens[2]).to.deep.equal(expectedOpcodeToken);
      expect(tokens[3]).to.deep.equal(expectedCommentToken);
    });
  });
  ...
  ['clr'].forEach(opcode => {
    ...
    it(`Operand opcode ${opcode}, operand returns opcode, operand tokens`, () => {
      ...
      expect(tokens.length).to.equal(3);
      expect(tokens[0]).to.deep.equal(expectedOpcodeToken);
      expect(tokens[1]).to.deep.equal(expectedAllOperandToken);
      expect(tokens[2]).to.deep.equal(expectedOperandToken);
    });
    ...
    it(`Symbol, operand opcode ${opcode}, comment returns symbol, opcode, operand, comment tokens`, () => {
      ...
      expect(tokens[0]).to.deep.equal(expectedSymbolToken);
      expect(tokens[1]).to.deep.equal(expectedOpcodeToken);
      expect(tokens[2]).to.deep.equal(expectedAllOperandToken);
      expect(tokens[3]).to.deep.equal(expectedOperandToken);
      expect(tokens[4]).to.deep.equal(expectedCommentToken);
    });
    ...
    it(`Immediate operand returns opcode, Operator, numeric token`, () => {
      ...
      expect(tokens[0]).to.deep.equal(expectedOpcodeToken);
      expect(tokens[1]).to.deep.equal(expectedAllOperandToken);
      expect(tokens[2]).to.deep.equal(expectedImmediateToken);
      expect(tokens[3]).to.deep.equal(expectedNumberToken);
    });
    ...
    it(`Immediate reference operand returns opcode, Operator, numeric token`, () => {
      ...
      expect(tokens[1]).to.deep.equal(expectedAllOperandToken);
      expect(tokens[2]).to.deep.equal(expectedImmediateToken);
      expect(tokens[3]).to.deep.equal(expectedReferenceToken);
    });
    ...
    it(`Immediate register inc operand returns opcode, Operator, numeric token`, () => {
      ...
      expect(tokens[0]).to.deep.equal(expectedOpcodeToken);
      expect(tokens[1]).to.deep.equal(expectedAllOperandToken);
      expect(tokens[2]).to.deep.equal(expectedCommaToken);
      expect(tokens[3]).to.deep.equal(expectedRegisterToken);
      expect(tokens[4]).to.deep.equal(expectedIncrementToken);
    });
  ...
  ['fcc', 'fcn', 'fcs'].forEach(pseudo => {
    it(`${pseudo} string, operand returns opcode, string tokens`, () => {
      ...
      expect(tokens.length).to.equal(2);
      expect(tokens[0]).to.deep.equal(expectedOpcodeToken);
      expect(tokens[1]).to.deep.equal(expectedOperandToken);
    });
  });
  ...
  it('pramga string, returns opcode, multiple pragmas', () => {
    ...
    expect(tokens.length).to.equal(4);
    expect(tokens[0]).to.deep.equal(expectedOpcodeToken);
    expect(tokens[1]).to.deep.equal(expectedFirstPragmaToken);
    expect(tokens[2]).to.deep.equal(expectedColonToken);
    expect(tokens[3]).to.deep.equal(expectedSecondPragmaToken);
  });
  ...
  it('opcode struct.property, returns opcode, struct, dot, porperty', () => {
    ...
    expect(tokens.length).to.equal(5);
    expect(tokens[0]).to.deep.equal(expectedOpcodeToken);
    expect(tokens[1]).to.deep.equal(expectedVariableToken);
    expect(tokens[2]).to.deep.equal(expectedStructToken);
    expect(tokens[3]).to.deep.equal(expectedDotToken);
    expect(tokens[4]).to.deep.equal(expectedPropertyToken);
  });
